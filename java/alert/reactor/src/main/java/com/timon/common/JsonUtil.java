...
public class JsonUtil {
    ...
    public static JSONObject read(String file){
        ...
        String name = DataUtil.szDate(new Date().getTime(),"-yyyy-MM-dd_HHmmssZ");
        name="";
        File f = new File("target/" + file + name +".json");
        try ( FileOutputStream fs = new FileOutputStream(f) ) {
            return objectMapper.readValue(f, JSONObject.class);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public static DocumentContext getContext(Object o){
        Configuration config = Configuration.defaultConfiguration()
                .addOptions(Option.DEFAULT_PATH_LEAF_TO_NULL)
                .addOptions(Option.SUPPRESS_EXCEPTIONS);
        DocumentContext dc  = JsonPath.using(config).parse(toJson(o));
        return dc;
    }
